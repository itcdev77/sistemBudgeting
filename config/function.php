...
function list_barang()
{
    ...
    while ($row = mysqli_fetch_array($kategori)) {
        $barang = mysqli_query($con, "SELECT x.*,x1.nama_merek FROM prodev x JOIN merek x1 ON x1.idmerek=x.merek_id WHERE kategori_id='" . $row['idkategori'] . "' ORDER BY deskripsi ASC");
        $opt .= "<optgroup label=\"" . $row['nama_kategori'] . " | " . $row['keterangan'] . "\">";
        while ($row2 = mysqli_fetch_array($barang)) {
            $opt .= "<option value=\"" . $row2['idbarang'] . "\">" . $row2['deskripsi'] . " - " . $row2['nama_merek'] . "</option>";
        }
        $opt .= "</optgroup>";
    }
    return $opt;
